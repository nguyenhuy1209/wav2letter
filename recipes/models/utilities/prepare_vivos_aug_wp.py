...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dst", help="data destination directory", default="./vivos_aug"
    )
    parser.add_argument(
        "--model_dst",
    ...
    prefix = "vivos_aug-train-all-unigram-{}".format(num_wordpieces)
        ...
        for path in subpaths:
            with open(os.path.join(lists_path, path + ".lst"), "r", encoding="utf-8") as flist:
                for line in flist:
                    ...
                    word_dict[path].update(transcription)
    ...
    for nbest in args.nbest.split(","):
        nbest = int(nbest)
        lexicon_name = "vivos_aug-train+dev-unigram-{sz}-nbest{n}.lexicon".format(
            sz=num_wordpieces, n=nbest
        )
        lexicon_name_train = "vivos_aug-train-unigram-{sz}-nbest{n}.lexicon".format(
            sz=num_wordpieces, n=nbest
        )
